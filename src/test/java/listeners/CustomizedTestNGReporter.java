...
public class CustomizedTestNGReporter implements IReporter{
    ...
    private String initReportTemplate() {
        ...
        byte[] reportTemplate;
        try {
            reportTemplate = Files.readAllBytes(Paths.get("reportTemplate.html"));
            template = new String(reportTemplate, "UTF-8");
        } catch (IOException e) {
            Reporter.log("Problem initializing template:" + e.getMessage(), 1);
        }
        return template;
    }
    ...
    private void saveReportTemplate(String outputDirectory, String reportTemplate) {
        ...
        try {
            ...
            Reporter.log("Problem saving template:" + e.getMessage(), 1);
        }
    }
}